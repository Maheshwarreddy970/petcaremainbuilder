        ...
        <section id='comparison' className={cn("py-20 w-full overflow-hidden", data.section?.className)} style={{ backgroundColor: data.section?.bg || data.bg || '#ffffff' }}>
            <div className="max-w-7xl mx-auto px-6 md:px-12 flex flex-col items-center">
                ...
                <div className="relative w-full max-w-[950px] mx-auto flex flex-col md:flex-row items-stretch">
                    <div className={cn("w-full md:w-1/2 border border-gray-200 border-b-0 md:border-b md:border-r-0 p-8 md:p-10 lg:p-12 rounded-t-[22px] md:rounded-t-none md:rounded-l-[22px]", data.leftColumn?.className)} style={{ backgroundColor: data.leftColumn?.bg || data.leftBg }}>
                        <h3 className="font-medium text-[24px] leading-[1.3] tracking-[-0.7px] mb-6" style={{ color: data.leftColumn?.textColor || data.leftText }}>Other offers</h3>
                        <ul className="flex flex-col">
                            {(data.leftColumn?.offers || data.otherOffers)?.map((item: string, index: number) => (
                                <li key={index} className="flex items-center gap-4 py-[18px] border-b border-gray-200/50 last:border-b-0">
                                    <XCircle className="w-5 h-5 flex-shrink-0 stroke-[1.5]" style={{ color: data.leftColumn?.iconColor || data.leftIcon }} />
                    ...
                    <div className={cn("w-full md:w-1/2 p-8 md:p-10 lg:p-12 rounded-b-[22px] md:rounded-r-[22px]", data.rightColumn?.className)} style={{ backgroundColor: data.rightColumn?.bg || data.rightBg }}>
                        <h3 className="font-medium text-[24px] leading-[1.3] tracking-[-0.7px] mb-6" style={{ color: data.rightColumn?.textColor || data.rightText }}>Your Offers</h3>
                        <ul className="flex flex-col">
                            {(data.rightColumn?.offers || data.petocareOffers)?.map((item: string, index: number) => (
                                <li key={index} className="flex items-center gap-4 py-[18px] border-b border-white/20 last:border-b-0">
                                    <CheckCircle className="w-5 h-5 flex-shrink-0 stroke-[1.5]" style={{ color: data.rightColumn?.iconColor || data.rightIcon }} />
